...
//    [fromLabel setFrame:frame];
    ...
    if (![GAHelper trackEventWithCategory:@"Route" withAction:@"Search" withLabel:@"" withValue:0]) {
...
- (IBAction)findRoute:(id)sender {
    ...
    if ([[self.fromData objectForKey:@"name"] isEqualToString:CURRENT_POSITION_STRING] == NO) {
        if (![GAHelper trackEventWithCategory:@"Route" withAction:@"From" withLabel:[self.fromData objectForKey:@"name"] withValue:0]) {
            debugLog(@"error in trackEvent");
        }
    }
...
//    [r findNearestPointForStart:[self.fromData objectForKey:@"location"] andEnd:[self.toData objectForKey:@"location"]];
    ...
    debugLog(@"%@", st);
    if (![GAHelper trackEventWithCategory:@"Route:" withAction:@"Finder" withLabel:st withValue:0]) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self isCountButton:indexPath]) {
        ...
        if (indexPath.section == 0) {
            if (![GAHelper trackEventWithCategory:@"Route" withAction:@"Search" withLabel:@"Favorites" withValue:0]) {
                debugLog(@"error in trackEvent");
            }
        } else {
            if (![GAHelper trackEventWithCategory:@"Route" withAction:@"Search" withLabel:@"Recent" withValue:0]) {
                debugLog(@"error in trackEvent");
            }
        }
        NSDictionary * currentRow = [[self.groupedList objectAtIndex:indexPath.section] objectAtIndex:indexPath.row];
